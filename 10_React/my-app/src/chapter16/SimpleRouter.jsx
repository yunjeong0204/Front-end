...
function SimpleRouter(props) {
  return (
    <BrowserRouter>
      <Routes>
        {/* index: index route(여기선 default child route) */}
        {/* 경로값 없이 루트로 접속하게 되면 index라는 prop을 가진 Route로 라우팅 */}

        {/* path는 경로를 의미 */}
        <Route path='/'  element={<MainPage />} /> {/* /로 접속하면 MainPage컴포넌트가 렌더링 */}
        <Route path='/places' element={<PlacePage />} /> {/* /places로 접속하면 PlacePage컴포넌트가 렌더링 */}
